...
class ProfileCounter:
    ...
    def __init__(self, matrix: pd.DataFrame, assay_cols: list[str]):
        ...
        self.arr = np.vstack(coded).T.astype(np.int64, copy=False) if coded else np.empty((matrix.shape[0], 0), dtype=np.int64)
        if self.arr.shape[1]:
...
def encode_phenotypes(df: pd.DataFrame, assay_cols: list[str]) -> pd.DataFrame:
    ...
    for col in assay_cols:
        ...
        is_continuous = numeric.notna().sum() >= max(10, int(0.80 * len(s))) and numeric.nunique(dropna=True) > 8
        if is_continuous:
            encoded[col] = discretize_continuous(s)
        else:
            encoded[col] = s.astype("string").str.strip().str.replace(r"\s+", "_", regex=True).str.lower().fillna("missing")
    return encoded
...
def greedy_accumulation(matrix: pd.DataFrame, assay_cols: list[str], counter: ProfileCounter | None = None) -> pd.DataFrame:
...
def exact_minimal_sets(matrix: pd.DataFrame, assay_cols: list[str], thresholds=(0.80, 0.90, 1.00), counter: ProfileCounter | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    counter = counter or ProfileCounter(matrix, assay_cols)
    full = counter.count(assay_cols)
    summary_records = []
    set_records = []
    for threshold in thresholds:
        ...
        for k in range(1, len(assay_cols) + 1):
            for combo in itertools.combinations(assay_cols, k):
                ...
                if profiles >= target:
                    found.append(combo)
                    found_profiles = profiles if found_profiles is None else max(found_profiles, profiles)
            if found:
                break
...
def shapley_like_contribution(matrix: pd.DataFrame, assay_cols: list[str], n_permutations: int = 1000, random_state: int = 1) -> pd.DataFrame:
    ...
    return pd.DataFrame(records).sort_values("mean_marginal_gain", ascending=False).reset_index(drop=True)


def bootstrap_stability(matrix: pd.DataFrame, assay_cols: list[str], n_iterations: int = 200, random_state: int = 2) -> tuple[pd.DataFrame, pd.DataFrame]:
    rng = np.random.default_rng(random_state)
    ...
    return pd.DataFrame(summary), pd.DataFrame(inc_records).sort_values("k90_inclusion_frequency", ascending=False).reset_index(drop=True)


def column_permutation_null(matrix: pd.DataFrame, assay_cols: list[str], n_iterations: int = 200, random_state: int = 3) -> pd.DataFrame:
    rng = np.random.default_rng(random_state)
    records = []
    ...
    observed_k100 = int(observed_exact.loc[observed_exact["target_fraction"] == 1.0, "minimal_n_assays"].iloc[0])
    ...
    for i in range(n_iterations):
        ...
        k100 = int(exact_summary.loc[exact_summary["target_fraction"] == 1.0, "minimal_n_assays"].iloc[0])
        top_gain_fraction = float(greedy.iloc[0]["gain"] / max(full - 1, 1))
        records.append({"iteration": i + 1, "null_exact_k100": k100, "null_top_gain_fraction": top_gain_fraction})
    out = pd.DataFrame(records)
    out.attrs["observed_exact_k100"] = observed_k100
    out.attrs["observed_top_gain_fraction"] = float(observed_greedy.iloc[0]["gain"] / max(full - 1, 1))
    return out
