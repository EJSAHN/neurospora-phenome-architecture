...
def main() -> None:
    ...
    summary = pd.DataFrame([
        {
            ...
            "published_clusters": matrix["published_cluster"].nunique(),
    ...
    shapley = shapley_like_contribution(assay_matrix, assay_cols, args.shapley_permutations, args.random_state)
    boot_summary, boot_inclusion = bootstrap_stability(assay_matrix, assay_cols, args.bootstrap_iterations, args.random_state + 1)
    null = column_permutation_null(assay_matrix, assay_cols, args.null_iterations, args.random_state + 2)
    redundancy = pairwise_redundancy(assay_matrix, assay_cols)

    observed_exact_k100 = int(exact_summary.loc[exact_summary["target_fraction"] == 1.0, "minimal_n_assays"].iloc[0])
    null_summary = pd.DataFrame([
        {
            ...
            "empirical_p_lower_or_equal": (1 + (null["null_exact_k100"] <= observed_exact_k100).sum()) / (len(null) + 1),
            "empirical_p_upper_or_equal": (1 + (null["null_exact_k100"] >= observed_exact_k100).sum()) / (len(null) + 1),
        }
    ])

    # Simple role table for assays.
    merged = shapley.merge(greedy[["assay_added", "step"]].rename(columns={"assay_added": "assay", "step": "greedy_step"}), on="assay", how="left")
