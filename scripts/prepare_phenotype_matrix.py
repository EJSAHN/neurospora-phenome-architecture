...
def main() -> None:
    ...
    if main_file is None:
        raise FileNotFoundError("Carrillo Additional file 1 was not found in data/raw/carrillo_2020")
    ...
    raw = raw.rename(columns={"NCU Number": "ncu_id", "Consolidated Gene Names": "gene_name", "Cluster": "published_cluster"})

    annot = phen[["NCU Number", "Consolidated Gene Names", "Cluster"]].rename(
        columns={"NCU Number": "NCU Number", "Consolidated Gene Names": "gene_name", "Cluster": "published_cluster"}
    )
    product_small = product.rename(columns={"Product Description": "product_description"})
    features_small = features.rename(columns={"Gene Classification": "gene_classification", "# of Phosphorylation Sites": "n_phosphorylation_sites", "# Transmembrane Helices": "n_transmembrane_helices"})
    ...
    annot = annot.merge(product_small[["NCU Number", "product_description"]], on="NCU Number", how="left")
    annot = annot.merge(features_small[["NCU Number", "gene_classification", "n_phosphorylation_sites", "n_transmembrane_helices"]], on="NCU Number", how="left")
    ...
    annot["is_metabolic_gene_carrillo_sheet"] = annot["is_metabolic_gene_carrillo_sheet"].fillna(False)
    ...
    if yeast_file is not None:
        ...
        yeast = yeast.rename(columns={"Gene Number": "ncu_id", "Gene Name": "yeast_sheet_gene_name"})
        yeast["has_yeast_ortholog"] = yeast[[c for c in yeast.columns if str(c).startswith("Yeast Ortholog")]].notna().any(axis=1)
    ...
    assay_catalog = pd.DataFrame({
        ...
        "assay_type": ["continuous_binned" if c in ["Basal hyphae growth rate (mm/day)", "Aerial hyphae height (total mm)"] else "categorical" for c in phenotype_cols],
    })
