#!/usr/bin/env python
"""Analyze reduced-panel recovery, reference-cluster structure, and gene-class enrichment."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze phenotype-panel architecture")
    ...
    parser.add_argument("--assay-permutation-null-iterations", type=int, default=-1)
    parser.add_argument("--random-seed", type=int, default=20260601)
    parser.add_argument("--max-minimal-sets", type=int, default=5000)
...
def assay_columns(discrete: pd.DataFrame, catalog: Optional[pd.DataFrame]) -> List[str]:
    if catalog is not None and "assay" in catalog.columns:
        assays = [
            a for a in catalog["assay"].dropna().astype(str).tolist() if a in discrete.columns
        ]
        if assays:
            return assays
        ...
        rows.append(
            {
                ...
                **{f"cluster_{k}": v for k, v in met.items()},
                ...
                "cluster_ari_fraction_of_full": (
                    met["ari"] / full_metrics["ari"] if full_metrics["ari"] > 0 else np.nan
                ),
                "cluster_purity_fraction_of_full": (
                    met["purity"] / full_metrics["purity"] if full_metrics["purity"] > 0 else np.nan
                ),
                "cluster_v_fraction_of_full": (
                    met["v_measure"] / full_metrics["v_measure"]
                    if full_metrics["v_measure"] > 0
                    else np.nan
                ),
...
def summarize_exact_minimal(
    subsets: pd.DataFrame, full_metrics: Dict[str, float], max_sets: int
                ...
                rows.append(
                    {
                        ...
                        "metric_column": col,
                        "full_metric_value": full,
                        "target_metric_value": target,
                        "minimal_n_assays": np.nan,
                        "n_equivalent_minimal_sets": 0,
                        "example_assay_set": "",
                        "best_metric_at_minimal_size": np.nan,
                        "best_profile_fraction_at_minimal_size": np.nan,
                        "min_default_labor_cost_at_target": np.nan,
            ...
            rows.append(
                {
                    ...
                    "min_default_labor_cost_at_target": float(
                        elig["total_default_labor_cost"].min()
                    ),
        ...
        rows.append(
            {
                "objective": objective,
                "step": step,
                "added_assay": enc.assays[i],
                "mask": selected_mask,
                "selected_assays": ";".join(selected),
                "n_profiles": nprof,
                "profile_fraction_of_full": nprof / full_profiles,
                "cluster_nmi": nmi,
                "cluster_nmi_fraction_of_full": nmi / full_nmi if full_nmi > 0 else np.nan,
                "cumulative_default_labor_cost": cost,
                "marginal_default_labor_cost": mcost,
                "marginal_profile_gain": nprof - prev_profiles,
                "marginal_cluster_nmi_gain": nmi - prev_nmi,
                "profiles_per_default_labor_cost": (
                    (nprof - prev_profiles) / mcost if mcost > 0 else np.nan
                ),
                "nmi_gain_per_default_labor_cost": (
                    (nmi - prev_nmi) / mcost if mcost > 0 else np.nan
                ),
            }
        )
...
def shapley_cluster(
    enc: EncodedPhenome, permutations: int, rng: np.random.Generator
) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "mean_marginal_cluster_nmi_gain": arr.mean(),
                "sd_marginal_cluster_nmi_gain": arr.std(ddof=1) if len(arr) > 1 else 0.0,
                "median_marginal_cluster_nmi_gain": np.median(arr),
...
def bootstrap_reduced(
    ...
    candidates = []
    for obj, frac in [
        ("profile_fraction", 0.80),
        ("profile_fraction", 0.90),
        ("cluster_nmi", 0.95),
        ("cluster_nmi", 0.99),
    ]:
        hit = exact_summary[
            (exact_summary["objective"] == obj)
            & np.isclose(exact_summary["target_fraction_of_full"], frac)
        ]
        if not hit.empty and hit.iloc[0]["example_assay_set"]:
            # recover mask from subset table
            mask = int(
                subsets.loc[subsets["assay_set"] == hit.iloc[0]["example_assay_set"], "mask"].iloc[
                    0
                ]
            )
            ...
            rows.append(
                {
                    ...
                    "subset_name": name,
                    "mask": mask,
                    "n_assays": len(enc.mask_to_assays(mask)),
                    "assay_set": ";".join(enc.mask_to_assays(mask)),
                    "bootstrap_full_cluster_nmi": full_nmi,
                    "bootstrap_subset_cluster_nmi": nmi,
                    "subset_fraction_of_full_nmi": nmi / full_nmi if full_nmi > 0 else np.nan,
...
def null_cluster(
    ...
    for k, g in subsets.groupby("n_assays"):
        ...
        best_prof = g.sort_values(
            ["profile_fraction_of_full", "cluster_nmi"], ascending=False
        ).iloc[0]
    ...
    for it in range(label_null_iters):
        ...
        for name, mask in candidates:
            ...
            rows.append(
                {
                    "iteration": it + 1,
                    "null_type": "cluster_label_shuffle",
                    "candidate_name": name,
                    "mask": mask,
                    "n_assays": len(enc.mask_to_assays(mask)),
                    "assay_set": ";".join(enc.mask_to_assays(mask)),
                    "observed_cluster_nmi": observed[mask],
                    "null_cluster_nmi": val,
                }
            )
    for it in range(colperm_iters):
        ...
        for name, mask in candidates:
            ...
            rows.append(
                {
                    "iteration": it + 1,
                    "null_type": "assay_column_permutation",
                    "candidate_name": name,
                    "mask": mask,
                    "n_assays": len(enc.mask_to_assays(mask)),
                    "assay_set": ";".join(enc.mask_to_assays(mask)),
        ...
        summary_rows.append(
            {
                ...
                "candidate_name": name,
                "mask": int(mask),
                "n_assays": int(g["n_assays"].iloc[0]),
                "assay_set": g["assay_set"].iloc[0],
                "observed_cluster_nmi": obs,
                "null_mean_cluster_nmi": vals.mean(),
...
def build_gene_classes(annot: pd.DataFrame) -> pd.DataFrame:
    ...
    yeast = (
        ...
        if "has_yeast_ortholog" in annot
        else pd.Series(False, index=annot.index)
    )
    out["yeast_ortholog"] = yeast
    out["no_yeast_ortholog"] = ~yeast
    out["TF"] = cls.eq("TF") | desc.str.contains(
        "transcription factor|zn2cys6|c2h2|bzip|myb|homeobox|ap2", regex=True
    )
    out["GPCR"] = (
        cls.eq("GPCR")
    ...
    out["ser_thr_kinase"] = cls.eq("STKIN") | desc.str.contains(
        "serine/threonine|protein kinase|map kinase|kinase", regex=True
    )
    out["phosphatase"] = cls.eq("PASE") | desc.str.contains("phosphatase", regex=True)
    tm = pd.to_numeric(
        annot.get("n_transmembrane_helices", pd.Series([0] * len(annot))), errors="coerce"
    ).fillna(0)
    ph = pd.to_numeric(
        annot.get("n_phosphorylation_sites", pd.Series([0] * len(annot))), errors="coerce"
    )
    out["transmembrane_any"] = tm >= 1
    out["transmembrane_5plus"] = tm >= 5
    cutoff = ph.dropna().quantile(0.75) if ph.notna().any() else np.inf
    out["high_phosphorylation_q75"] = ph.fillna(0) >= cutoff
    return out
...
def enrichment_tables(
    ...
    class_cols = [
        c
        for c in gene_classes.columns
        if c not in {"ncu_id", "gene_name"} and data[c].fillna(False).astype(bool).sum() >= 5
    ]
    ...
    for gc in class_cols:
        ...
        for clus in sorted(data["published_cluster"].unique()):
            ...
            cluster_rows.append(
                {
                    "gene_class": gc,
                    "published_cluster": clus,
                    "n_cluster_genes": int(inc.sum()),
                    "n_class_genes": int(mem.sum()),
                    "class_in_cluster": x1,
                    "class_outside_cluster": x0,
                    "nonclass_in_cluster": y1,
                    "nonclass_outside_cluster": y0,
                    "class_rate_in_cluster": x1 / max(x1 + y1, 1),
                    "class_rate_outside_cluster": x0 / max(x0 + y0, 1),
                    "odds_ratio": odds,
                    "log2_odds_ratio_corrected": log2_or_corrected(x1, x0, y1, y0),
                    "fisher_p": p,
                }
            )
    assay_enrich, axis_enrich, cluster_enrich = (
    ...
    for df in [assay_enrich, axis_enrich, cluster_enrich]:
        if not df.empty:
            df["fdr_q"] = bh_fdr(df["fisher_p"])
    exposure = data[
        [
            "ncu_id",
            "gene_name",
            "published_cluster",
            "growth_abnormal_count",
            "asexual_development_abnormal_count",
            "sexual_development_abnormal_count",
            "total_abnormal_count",
            "any_abnormal",
        ]
        + class_cols
    ].copy()
    exposure["axis_exposure_signature"] = (
        exposure["growth_abnormal_count"].astype(str)
        + "|"
        + exposure["asexual_development_abnormal_count"].astype(str)
        + "|"
        + exposure["sexual_development_abnormal_count"].astype(str)
    )
    ...
    for col in ["total_exposure_label", "axis_exposure_signature"]:
        ...
        exp_rows.append(
            {
                "table": "exposure_cluster_metrics",
                "exposure_model": col,
                "n_exposure_states": int(exposure[col].nunique()),
                **{f"cluster_{k}": v for k, v in met.items()},
            }
        )
    for gc in class_cols:
        mem = exposure[gc].fillna(False).astype(bool)
        if mem.sum() < 5 or (~mem).sum() < 5:
            continue
        try:
            u, p = stats.mannwhitneyu(
                exposure.loc[mem, "total_abnormal_count"],
                exposure.loc[~mem, "total_abnormal_count"],
            )
        except Exception:
            u, p = np.nan, np.nan
        exp_rows.append(
            {
                "table": "exposure_class_stats",
                "gene_class": gc,
                "n_class_genes": int(mem.sum()),
                "mean_total_abnormal_count_class": exposure.loc[mem, "total_abnormal_count"].mean(),
                "mean_total_abnormal_count_nonclass": exposure.loc[
                    ~mem, "total_abnormal_count"
                ].mean(),
                "median_total_abnormal_count_class": exposure.loc[
                    mem, "total_abnormal_count"
                ].median(),
                "median_total_abnormal_count_nonclass": exposure.loc[
                    ~mem, "total_abnormal_count"
                ].median(),
                "mannwhitney_u": u,
                "mannwhitney_p": p,
...
def class_specific_compression(enc: EncodedPhenome, gene_classes: pd.DataFrame) -> pd.DataFrame:
    data = enc.discrete[["ncu_id", "gene_name"]].merge(
        gene_classes, on=["ncu_id", "gene_name"], how="left"
    )
    rows = []
    for gc in [c for c in gene_classes.columns if c not in {"ncu_id", "gene_name"}]:
        ...
        for frac in [0.80, 0.90, 1.00]:
            ...
            best = sorted(
                [(mask, npf) for mask, npf in elig if len(enc.mask_to_assays(mask)) == kmin],
                key=lambda x: x[1],
                reverse=True,
            )[0]
            rows.append(
                {
                    "gene_class": gc,
                    "n_class_genes": len(row_idx),
                    "full_profiles_within_class": full_prof,
                    "target_profile_fraction": frac,
                    "minimal_n_assays": kmin,
                    "example_assay_set": ";".join(enc.mask_to_assays(best[0])),
                    "profiles_at_minimal_size": best[1],
...
def main() -> None:
    ...
    if "published_cluster" not in discrete:
        raise ValueError("published_cluster column is required")

    encoded = EncodedPhenome(discrete, assays)
    gene_classes = build_gene_classes(annot)
    abnormal = abnormality_table(discrete, assays)

    subsets, full_metrics = build_subset_metrics(encoded)
    exact_summary, exact_sets = summarize_exact_minimal(
        subsets, full_metrics, args.max_minimal_sets
    )
    greedy_profiles = greedy_curve(encoded, "profiles")
    greedy_cluster = greedy_curve(encoded, "cluster_nmi")
    greedy_cost = greedy_curve(encoded, "cost_aware_profiles")
    cluster_shapley = shapley_cluster(
        encoded, args.shapley_permutations, np.random.default_rng(args.random_seed + 1)
    )
    bootstrap = bootstrap_reduced(
        encoded,
        exact_summary,
        subsets,
        args.bootstrap_iterations,
        np.random.default_rng(args.random_seed + 2),
    )
    column_null_iterations = (
        min(args.null_iterations, 200)
        if args.assay_permutation_null_iterations < 0
        else args.assay_permutation_null_iterations
    )
    null_summary, null_iterations = null_cluster(
        encoded,
        subsets,
        args.null_iterations,
        column_null_iterations,
        np.random.default_rng(args.random_seed + 3),
    )

    assay_enrichment, axis_enrichment, cluster_enrichment, exposure, exposure_summary = (
        enrichment_tables(discrete, annot, gene_classes, abnormal, assays)
    )
    class_compression = class_specific_compression(encoded, gene_classes)
    marginal_cost = pd.concat(
        [
            greedy_profiles.assign(curve="greedy_profile_resolution"),
            greedy_cluster.assign(curve="greedy_cluster_nmi"),
            greedy_cost.assign(curve="cost_aware_profile_resolution"),
        ],
        ignore_index=True,
    )
        ...
        [
            {
                ...
                "n_assays": len(assays),
                "n_reference_clusters": int(discrete["published_cluster"].nunique()),
                "full_discrete_profiles": encoded.n_profiles(encoded.full_mask),
                "full_profile_cluster_nmi": full_metrics["nmi"],
                "full_profile_cluster_ari": full_metrics["ari"],
                "full_profile_cluster_purity": full_metrics["purity"],
                "shapley_permutations": args.shapley_permutations,
                "bootstrap_iterations": args.bootstrap_iterations,
                "cluster_label_null_iterations": args.null_iterations,
                "assay_permutation_null_iterations": column_null_iterations,
    ...
    sheets = {
        ...
        "cluster_all_subsets": subsets.sort_values(
            ["n_assays", "cluster_nmi", "profile_fraction_of_full"],
        ...
        "cluster_null_iterations": null_iterations,
        "gene_class_membership": gene_classes,
        "gene_class_assay_enrich": (
            assay_enrichment.sort_values("fdr_q")
            if not assay_enrichment.empty
            else assay_enrichment
        ),
        "gene_class_axis_enrich": (
            axis_enrichment.sort_values("fdr_q") if not axis_enrichment.empty else axis_enrichment
        ),
        "gene_class_cluster_enrich": (
            cluster_enrichment.sort_values("fdr_q")
            if not cluster_enrichment.empty
            else cluster_enrichment
        ),
        ...
        "assay_catalog": (
            ...
            else pd.DataFrame({"assay": assays, "axis": [assay_axis(assay) for assay in assays]})
        ),
    }
    output = (
        Path(args.output_workbook)
        if args.output_workbook
        else tables / "phenome_architecture_results.xlsx"
    )
    ...
    axis_enrichment.to_csv(source / "gene_class_axis_enrichment.csv", index=False)
    exposure.to_csv(source / "exposure_gene_table.csv", index=False)

    print(f"Functional-architecture analysis complete: {output}")
